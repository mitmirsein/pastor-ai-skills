...
3. **메타데이터 우선:** 인덱스·manifest와 개별 파일 frontmatter로 검색한다. 인덱스가 있어도 같은 폴더의 누락 파일을 보완하며 경로로 중복 제거한다. 사용자가 특정 결과를 더 보겠다고 후속 요청할 때에만 v{NN} 파일 본문을 연다.
...
| **시기 지정** | 날짜·기간 언급 ("한 달 전", "3월 주일 설교") | `outputs/sermons/_index.md` date 필터 (v2.18) → 부재 시 개별 산출물 frontmatter `date` 필터링(미기재는 날짜 불명으로 별도 표시) |
...
본문 질의는 책의 한글명·통용 약어·영문 slug를 같은 책으로 정규화하고 장·절 범위의 겹침을 확인한다(장 경계 포함). `tools/exegete/pastor_adapter.py`의 기존 본문 파서와 `outputs/sermons/_README.md` 표기를 참조한다. 검색 때문에 폴더 이름을 바꾸지 않는다. 상대 기간은 현재 날짜·시간대 기준의 실제 시작/종료일로 표시한다. 인덱스 유무와 관계없이 대상 루트의 파일 경로를 대조해 **이미 인덱스에 있는 폴더의 새 파일도** 보완한다. 기간 검색에는 manifest 갱신일을 사용하지 않는다. 과거 `04_church_admin` 자료는 읽기 호환만 유지하며 새 행정 생성을 추천하지 않는다.
...
3. 시기 질의: 개별 산출물 `date`가 질의 기간에 들어오는 것 우선
...
- **경계:** 일반 검색과 마찬가지로 v{NN} 파일의 frontmatter를 열 수 있다 — 단 **본문 내용은 읽지도 인용하지도 않는다**(제약 3·5의 정신 유지). `seed_refs` 또는 `source_refs`에 다른 본문의 뿌리 큐티가 있으면 교차 본문 가지로 표시한다. 온톨로지 확장 금지 불변 — 기계적 취합만, 주제가 비슷한 다른 본문을 임의로 붙이지 않는다.
...
1. **{passage_id 또는 토픽}** — [{파일명}]({실제로 확인한 파일 경로})
