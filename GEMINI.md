...
> *이전 버전: v2.18 (Closed Loops — 상류 자동 참조 · 회고 앞단 상기 · 칼럼 아카이브 조회 · 씨앗 접합 블록 · 발아 퍼널 · 구술성 렌즈 · 낭독 리허설 · 설교 인덱스)*
...
3. **최적 스킬 라우팅:** 5대 그룹 31개 스킬(+harness·lenses) 중 1개를 매칭하여 복사/붙여넣기용 완성 프롬프트를 제공합니다.
...
- `skills/` (전문가 길드)
    ...
    - `03_omni_publisher/`: 재생산 얇은 라우터 및 다매체 변환 (블로그, 칼럼, TTS, 카드뉴스) + 큐티 발아 칼럼(qt-to-column)·큐티 발아 성도묵상(qt-to-devotional, v2.16)
