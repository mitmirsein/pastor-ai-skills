...
> *이전 버전: v2.20 (Rhythm & Retrospect — 큐티 리듬 엔진 · 커버리지 조망 · 월간 형성 스냅샷 · 발행물 회고 · 지면 마감 리듬 · schema v3)*
...
- **네 번째 물길 — 큐티→성도 묵상 (v2.16):** `qt-to-devotional`이 목회자의 큐티/씨앗에서 성도용 묵상을 빚습니다(신규 스킬, 스킬 30개). 핵심은 **층위 분리 게이트** — 본문 통찰 층만 성도 눈높이로 번역하고, 목회자 내면 층(직무 성찰·자기 점검)은 기본 제외·옵트인이며 분류를 먼저 보여 목회자가 확정합니다. 실질은 통찰 층에서만, AI는 번역·구조만(새 통찰·일화 금지, "목회자는→우리는" 몰래 일반화 금지, 애매하면 내면 층). 산출물은 `qt_kind` 태그가 없어 발아 코퍼스에 섞이지 않고, stage는 파일 전용 `devotional_draft`(journal 불변, §3.1.1)입니다.
...
3. **최적 스킬 라우팅:** 5대 그룹 32개 스킬(+harness·lenses) 중 1개를 매칭하여 복사/붙여넣기용 완성 프롬프트를 제공합니다.
