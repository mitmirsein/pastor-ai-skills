# 🗓️ 주간 리듬 (Pastoral Rhythm) — v2.22 (첫 작업 진입·매일 큐티 층·발아 동선)
...
7. **첫 작업 우선**: journal이 비어 있거나 foundation 설정이 미확정이어도 현재 요청을 먼저 실행한다. 설정을 위해 작업을 막거나 전체 프로필을 다시 묻지 않으며, 필요한 값만 미확정으로 표시한다.
