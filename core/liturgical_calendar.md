...
<!-- Concierge 요약 헤더 (부트 시 이 블록 + §4 매핑 규칙 + §5 출력 양식만 로드) -->
> **요약 (v2.20 부트 경량화)**: `currentDate`를 §4 매핑 규칙에 통과시켜 **서구 절기 + 한국 절기 overlay**를 판정하고 §5 양식으로 헤더에 표시한다. 원칙: 절기는 제안이지 제약이 아니다(시리즈 진행 중이면 시리즈 우선), 한국 고유 절기는 1급 시민, 교파(`denomination`)에 따라 강조를 조절한다. **각 절기의 상세(추천 본문·톤·주제 흐름, §2~3)는 부트에서 읽지 않는다** — 판정된 절기로 본문·주제를 실제 추천할 때 그 절기의 절만 지연 로드한다.
<!-- /요약 헤더 -->
...
5. **미확정 설정을 추정하지 않는다.** `foundation.md`의 교단 값이 `confirmed_fields`에 없거나 템플릿 예시로 남아 있으면 교파별 가중치를 적용하지 않고 일반 절기 라벨만 표시한다. 첫 작업을 막기 위해 설정 전체를 다시 묻지 않는다.
