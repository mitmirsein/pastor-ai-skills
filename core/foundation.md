---
church_name: "OOO 교회"
pastor_name: "OOO 목사"
denomination: "대한예수교장로회(합동)" # 예: 통합, 합동, 감리교 등
attendance_avg: 100
location: "서울시 OOO구"
preferred_bible: "개역개정" # KRV
theological_orientation: "개혁주의" # 또는 복음주의, 웨슬리언 등
tone_preference: "따뜻하고 권위 있는" # 문체 선호도
qt_source_path: "" # (선택) 기존 큐티/묵상 노트 폴더 — 콤마로 다중 지정 가능. 상세: 아래 설정 가이드 §4
column_venues: [] # (선택, v2.16) 칼럼 지면 프로파일 — 상세: 아래 설정 가이드 §5
---

# ⚓ Pastor-KR Foundation

본 파일은 `Pastor-KR` 프로젝트의 **단일 진실 공급원(SSOT)**입니다. 모든 목회 지원 스킬은 목회자가 실제 확인한 YAML 값을 참조하여 결과물을 개인화합니다. 배포 템플릿의 교단·출석·소재지·신학 지향은 예시이며 확정된 사용자 정보가 아닙니다. 이번 요청의 명시값을 우선하고, `confirmed_fields`가 있으면 그 목록의 키만 확정 설정으로 사용합니다. 목록이 없는 과거 설정은 미확정으로 표시하되 결과에 영향을 주는 값만 확인합니다. 교회명·성함·인원·소재지 때문에 첫 작업을 차단하지 않습니다.

## 🛡️ 글로벌 할루시네이션 방지 원칙 (Anti-Hallucination Guardrails)

1. **정직한 불가지론 (Honest Agnosticism):** 데이터가 없거나 확실하지 않은 정보에 대해서는 "모름" 또는 "불확실함"을 명확히 밝히십시오. 억지로 설득력 있는 답변을 지어내는 것(Confabulation)을 금지합니다.
2. **원어 엄밀성:** 성경 원어(헬라어, 히브리어)는 반드시 공인된 사본(NA28, BHS)에 근거해야 하며, AI의 추측으로 파싱하거나 번역하지 마십시오.
3. **출처의 족보화:** 모든 신학적 견해는 가능한 한 특정 학파나 전통(예: 개혁주의, 루터교, 성공회 등)의 입맥을 명시하여 출처의 불분명함을 제거하십시오.
4. **이중 검수 (Double-Check):** 답변을 출력하기 전, 내부적으로 팩트체크를 1회 이상 수행한 후 최종 응답을 구성하십시오.

## 설정 가이드

1. 위 YAML 프론트매터의 내용을 목회자 본인의 상황에 맞게 수정하세요 (첫 설정은 `foundation-setup` 스킬 권장).
2. 각 스킬 실행 시 이 설정을 기반으로 '우리 교회 맞춤형' 톤앤매너가 적용됩니다.
3. 신학 용어의 표준 표기는 `data/terms/`에 용어 대조표를 넣으면 발행 전 감사(`sermon_audit` L2)가 자동 대조합니다 (`data/_README.md` 참고).
4. `qt_source_path`(선택): 기존 큐티/묵상 노트 폴더의 경로. **콤마로 여러 폴더를 지정할 수 있습니다** (예: `"~/Obsidian/묵상일지"` 또는 `"묵상일지/, 성경연구/"`). 비우면 `outputs/devotionals/`​만 발아 스캔 대상이 됩니다. 지정된 폴더의 큐티 원문은 **읽기 전용으로만** 스캔하며 수정·이동하지 않습니다.
5. `column_venues`(선택, v2.16 · v2.20): 칼럼을 싣는 **지면 프로파일** 목록. 설정하면 칼럼 스킬(`qt-to-column`·`sermon-to-column`)이 요청에 지면이 없고 선택이 필요할 때만 물으며, 이미 준 지면·분량·독자·톤은 재사용하고, `sermon_audit` 칼럼 프로파일(§4.4)이 지면 분량을 대조합니다. 비우면 현행 기본(약 1,000~1,500자)이 유지됩니다. *(v2.20)* 선택 필드 `cadence`(발행 주기: `weekly`/`monthly`/`adhoc`)·`deadline_rule`(마감 규칙, 자유 문자열)을 채우면 `weekly-briefing`이 임박 마감을 1줄 리마인드합니다 — 미설정이면 어떤 스킬도 묻지 않습니다(정직 폴백). 예:
   ```yaml
   column_venues:
     - name: "주보"
       length: "600자 내외"
       audience: "전 세대 성도"
       tone: "따뜻하고 간결"
       cadence: weekly            # (선택, v2.20) weekly | monthly | adhoc
       deadline_rule: "매주 금요일 정오"   # (선택, v2.20) 자유 문자열
     - name: "지역신문 기고"
       length: "2,000자 내외"
       audience: "비신자 포함 일반 독자"
       tone: "보편 언어, 교회 내부어 자제"
   ```

---
마지막 업데이트: 2026-07-02 (frontmatter 정화 — 가드레일을 본문으로 이동, YAML 파서 호환)
