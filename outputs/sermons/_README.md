...
- 새 작업이면 같은 스킬도 새 버전을 부여한다(원문 덮어쓰기 금지). 같은 저장의 실패 재시도는 `_hooks.md` §2.7에 따라 기존 operation과 경로를 재사용한다.
...
- `current_stage`는 본체 진행 상태다. 최신 파일의 stage를 복사하지 않고 `core/_hooks.md` §3.6의 전이 규칙을 적용한다. 재묵상·회고·리허설·파생 초안은 본체 불변이며 실제 선포/게시 사건만 해당 전이를 허용한다.
