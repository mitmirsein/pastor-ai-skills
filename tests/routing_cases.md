...
| 73 | (경계) "이 옛 초안 다듬어서 완성해줘" | 대필 거절 + 동반작성(sermon-outline-codraft) 안내 | 입양(보관) ≠ 개작·대필 |
...
| 76 | "오늘은 질문 없이 짧게 큐티 기록만 도와줘" | qt-companion (침묵 동행 light) | v2.17 — 강도 다이얼 |
| 77 | "설교 원고 다 썼는데, 내일 새벽에 소리 내 읽어보게 어색한 데 잡아줘" | sermon-rehearse | v2.18 — 낭독 워크스루(개작 요청 아님) |
| 78 | (경계) "이 원고 낭독하기 좋게 문장 다 고쳐 써줘" | 대필 거절 + sermon-rehearse(표지만) 안내 | v2.18 — 리허설(관찰) ≠ 개작 |
| 79 | "이 큐티로 칼럼 쓰되, 당회 관련 얘기는 알아서 걸러줘" | qt-to-column (1.5단계 층위 게이트 — 직무 기밀 기본 제외 확인) | v2.19 — 칼럼 층위 게이트 |
