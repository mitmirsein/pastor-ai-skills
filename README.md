...
- `qt-to-devotional.md` *(v2.16)*: **목회자의 큐티/씨앗에서 성도용 묵상**을 빚기 — 층위 분리 게이트(통찰 층만 번역, 내면 층 기본 제외·옵트인), 실질은 목회자 묵상에서
