...
## 🚀 쉬운 설치 및 시작하기 (Installation)

본 툴킷은 별도의 프로그램 설치가 필요 없는 **'지침 기반(Instruction-based)'** 시스템입니다. 아래 순서대로 따라 하시면 1분 안에 나만의 사역 파트너를 가동할 수 있습니다.

### 1. 리포지토리 다운로드 (Clone)
터미널에서 아래 명령어를 입력하거나, 우측 상단의 **[Code] -> [Download ZIP]**을 통해 다운로드하세요.
```bash
git clone https://github.com/mitmirsein/pastor-ai-skills.git
```

### 2. 나만의 환경 설정 (Foundation Setup)
`core/foundation.md` 파일을 열어 교회 이름, 목회자 성함, 교단 배경, 선호하는 번역본 등을 본인의 환경에 맞게 수정하세요. 이 설정이 모든 AI 응답의 **'기준(SSOT)'**이 됩니다.

### 3. AI에게 지식 주입 (Load Skills)
사용하시는 AI 에이전트(Cursor, Claude, ChatGPT 등)에게 다음과 같이 요청하세요.
> **"이 폴더 안의 `core/skills` 폴더에 있는 모든 `.md` 파일들을 네 지식으로 로드해 줘. 그리고 앞으로 내가 명령하는 스킬들을 그 지침에 맞춰 실행해줘."**
...
> **"방금 네가 내놓은 원어 분석과 주석적 견해를 `core/foundation.md`의 가드레일에 따라 재검토(Re-check)해 줘. 지어낸 부분이나 비약이 있다면 스스로 수정해."**
...
3.  **즉시 실행 가능성 (Portable):** 별도의 복잡한 설치 없이 지침을 읽게 하는 것만으로 바로 사역 파트너로 활용할 수 있습니다.
